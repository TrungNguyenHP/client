<template>
  <div class="select-none ">
    <div class="w-full max-w-md bg-white rounded-xl shadow-md p-8 mx-auto">
      <div class="text-center mb-6">
        <h1 class="text-3xl font-bold text-green-700">Khôi phục mật khẩu</h1>
        <p class="text-gray-600 mt-2">Nhập email để nhận hướng dẫn đặt lại mật khẩu</p>
      </div>
      <form @submit.prevent="submitEmail" class="space-y-5">
        <input
          type="email"
          v-model="email"
          placeholder="Email của bạn"
          class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500"
          required
        />
        <button
          type="submit"
          class="w-full bg-green-600 text-white py-3 rounded-lg font-semibold hover:bg-green-700 transition duration-200"
        >
          Gửi
        </button>
      </form>
      <p class="text-center text-sm text-gray-600 mt-6">
        <RouterLink to="/login" class="text-green-600 hover:underline font-medium">Quay lại đăng nhập</RouterLink>
      </p>
    </div>
  </div>
</template>

<script setup lang="ts">

import { ref } from 'vue'
import axios from 'axios'

const email = ref('')

const submitEmail = async () => {
  if (!email.value) {
    alert('Vui lòng nhập email.')
    return
  }

  try {
    const response = await axios.post('http://localhost:5246/api/customer/forgot_password', {
      email: email.value
    })

    alert(response.data || 'Hướng dẫn đặt lại mật khẩu đã được gửi đến email của bạn.')
    email.value = ''
  } catch (error: any) {
    alert(error.response?.data || 'Không thể gửi email. Vui lòng thử lại sau.')
  }
}
</script>


